...
export default function Verdict({
  ...
  const buildReport = useCallback(() => {
    ...
    if (totalVotes > 0) {
      ...
      for (const o of [...votingOptions].sort((a, b) => b.votes - a.votes)) {
        ...
        const bar = '█'.repeat(Math.round((o.votes / Math.max(totalVotes, 1)) * 20));
  ...
  const buildParticipation = useCallback(() => {
    ...
    const lines: string[] = [
      `${scenario.title} — Participation Log`,
      `${dateStr}`,
      '',
    ...
    for (const [name, data] of sorted) {
      ...
      for (const entry of data.entries) {
        lines.push(`    • ${entry.length > 120 ? entry.slice(0, 120) + '...' : entry}`);
      }
      lines.push('');
    }
  ...
  return (
    <div className="relative mx-auto max-w-3xl">
      ...
      <div className="relative space-y-6" style={{ zIndex: 1 }}>
            ...
            {isTie ? (
              <>
                ...
                <p className="mt-2 text-base" style={{ color: 'var(--text-secondary)' }}>
                  {sorted.filter((o) => o.votes === winner.votes).map((o) => o.label).join(' vs. ')} — {winner.votes} votes each
                </p>
              </>
            ) : (
              <>
        ...
        {/* Session Stats — clickable to expand analysis */}
        <div
